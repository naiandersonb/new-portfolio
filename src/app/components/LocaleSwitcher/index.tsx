...
export function LocaleSwitcher() {
  ...
  return (
    <div className="grid grid-cols-2 divide-x dark:divide-stone-700 divide-stone-400">
      <Link
        className={`px-4 ${locale === 'pt' ? 'dark:text-stone-50 text-stone-950' : 'text-neutral-500'}`}
        href={`/pt/${path}`}
        locale="pt"
      >
        BR
      ...
      <Link
        className={`px-4 ${locale === 'en' ? 'dark:text-stone-50 text-stone-950' : 'text-neutral-500'}`}
        href={`/en/${path}`}
        locale="en"
      >
        EN
